...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.map(elem => (elem, chars.count(e => e == elem))).distinct
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    ...
    val second = trees(1)
    trees.drop(1).drop(1).::(makeCodeTree(first, second))
    //    trees.filter(tree => tree != first || tree != second).sortBy(tree => weight(tree))
  }
  ...
  def until(single: (List[CodeTree]) => Boolean, comb: (List[CodeTree]) => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    def untilAcc(accTrees: List[CodeTree]): List[CodeTree] = {
      if (single(accTrees))
        accTrees
      else
        untilAcc(comb(accTrees))
    }
